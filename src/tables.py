...
def build_table_2_metrics(
    ...
    for cause in [1, 2]:
        ...
        cif_at_horizons = compute_cox_cif(
            surv_fns_k, surv_fns_overall, times_to_evaluate
        )
        ...
        if cause in models_fg_python:
            ...
            try:
                cif_final = fg_model.predict_cumulative_incidence(
                    X_test_scaled.values, times=np.array([df_train["time"].max()])
                )[:, -1]
            ...
            for t in times_to_evaluate:
                try:
                    risk_at_t_fg = fg_model.predict_cumulative_incidence(
                        X_test_scaled.values, times=np.array([t])
                    )[:, 0]
                    auc_t_fg, _ = cumulative_dynamic_auc(
            ...
            try:
                cif_brier = fg_model.predict_cumulative_incidence(
                    X_test_scaled.values, times=brier_times
                )
        ...
        for model_name, cif_pred in [
            ("Transformer A", cif_A_all.numpy()),
            ("Transformer B", cif_B_all.numpy()),
        ]:
...
def build_table_3_hyperparams(
    ...
    current_n_heads = (
        model_sig.parameters["n_heads"].default
        if "n_heads" in model_sig.parameters
        else "N/A"
    )
    current_num_layers = (
        model_sig.parameters["num_layers"].default
