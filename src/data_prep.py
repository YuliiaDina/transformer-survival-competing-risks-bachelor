...
def load_and_prepare_data(url="https://raw.githubusercontent.com/vincentarelbundock/Rdatasets/master/csv/survival/mgus2.csv"):
    df = pd.read_csv(url)
...
def split_and_scale_data(df, features):
    ...
    df_train, df_val = train_test_split(
        df_train_val, test_size=0.125, stratify=df_train_val['event_type'], random_state=42
    )
...
def prepare_tensor_loaders(X_train_s, df_train, X_val_s, df_val, X_test_s, df_test, bins, batch_size=64):
    ...
    train_loader = DataLoader(TensorDataset(X_tr, y_tr_e, y_tr_t), batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(TensorDataset(X_va, y_va_e, y_va_t), batch_size=batch_size, shuffle=False)
