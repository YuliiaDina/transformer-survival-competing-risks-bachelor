...
def plot_calibration_curves_survival(models_cox, model_A_hist, model_B_hist, X_test_scaled, X_test_tensor, df_test, bins, target_time=60):
    ...
    for cause in [1, 2]:
        ...
        # Variant A
        preds['Variant A (Стандартна)'] = cif_A_all[:, cause-1, target_bin_idx].numpy()

        # Variant B
        preds['Variant B (Монотонна)'] = cif_B_all[:, cause-1, target_bin_idx].numpy()

        colors = {'Cox': 'green', 'Variant A (Стандартна)': 'blue', 'Variant B (Монотонна)': 'red'}
        ...
        for name, risk_probs in preds.items():
            ...
            for b in range(5):
                ...
                if mask.sum() > 0:
                    ...
                    obs_incidence.append(get_observed_incidence(y_test_cause[mask], target_time))

            ax.plot(mean_pred, obs_incidence, marker='o', color=colors[name],
