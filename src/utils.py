...
def run_cif_checks(model: nn.Module, model_name: str, X_tensor: torch.Tensor) -> None:
    ...
    print(
        f" [i] Сума CIF на останньому інтервалі: середнє={mean_sum:.4f}, min={min_sum:.4f}, max={max_sum:.4f}"
    )

    # Ймовірність вижити S(t)
    print(
        f"  -> Це означає, що середня ймовірність пацієнта вижити без жодної події S(t) до кінця дослідження складає {(1-mean_sum)*100:.1f}%"
    )
